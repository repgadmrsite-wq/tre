...
(function () {
    ...
    // Convert Latin digits to Farsi digits for display
...
const reservationState = {
    ...
    // Skeleton and empty state helpers
    function showSkeleton(container, count = 1, type = 'card') {
        ...
        for (let i = 0; i < count; i++) {
            items += '<div class="skeleton-card"><div class="skeleton-img"></div><div class="skeleton-text"></div></div>';
        }
        container.innerHTML = items;
    }

    function emptyStateHTML(message, anchorId, ctaText) {
        const btn = anchorId ? `<button type="button" onclick="document.getElementById('${anchorId}').scrollIntoView({behavior:'smooth'})">${ctaText}</button>` : '';
        return `<div class="empty-state"><p>${message}</p>${btn}</div>`;
    }
    ...
    function initHeroSection() {
        ...
        if (quickBtn && quickModal) {
            let lastFocused;
            function trap(e){
                const focusable = quickModal.querySelectorAll('a[href],button,textarea,input,select,[tabindex]:not([tabindex="-1"])');
            ...
            function closeModal(){
                ...
                quickModal.removeEventListener('keydown', trap);
            ...
            quickBtn.addEventListener('click', () => {
                ...
                if (quickPrice && vehicle) quickPrice.textContent = `قیمت: ${toFarsiNumber(vehicle.price.toLocaleString())} تومان`;
                if (quickEditFields) quickEditFields.classList.add('hidden');
                lastFocused = document.activeElement;
                quickModal.classList.remove('hidden');
                quickModal.addEventListener('keydown', trap);
                const focusable = quickModal.querySelectorAll('a[href],button,textarea,input,select,[tabindex]:not([tabindex="-1"])');
            ...
            if (quickCancel) quickCancel.addEventListener('click', closeModal);
    ...
    function initializeBookingForm() {
        ...
        function renderVehicleList() {
            ...
            showSkeleton(vehicleListContainer, 3);
            setTimeout(() => {
                if (vehiclesData.length === 0) {
                    vehicleListContainer.innerHTML = emptyStateHTML('موتوری موجود نیست', 'ready', 'دیدن مدل‌های مشابه');
                    return;
                }
    ...
    function updateSummary() {
        ...
        if (reservationState.deliveryLocation) {
            ...
            const latFa = toFarsiNumber(lat);
            const lngFa = toFarsiNumber(lng);
            locationText = reservationState.deliveryLocationName
                ? `${reservationState.deliveryLocationName} (${latFa}, ${lngFa})`
                : `${latFa}, ${lngFa}`;
        }
        // Build summary HTML
        ...
        if (reservationState.duration === 'hourly') {
            ...
            summaryHTML += `<p><strong>بازه:</strong> ${toFarsiNumber(reservationState.startTime || '-')} تا ${toFarsiNumber(reservationState.endTime || '-')}</p>`;
        } else if (reservationState.duration === 'half-day') {
            summaryHTML += `<p><strong>تاریخ:</strong> ${reservationState.startDate ? formatJalali(reservationState.startDate) : '-'}</p>`;
            ...
            const timeLabel = reservationState.halfDayOption === 'morning'
                ...
                ? '۱۲:۰۰ - ۲۴:۰۰'
                : '-';
            summaryHTML += `<p><strong>بازه زمانی:</strong> ${toFarsiNumber(timeLabel)}</p>`;
        } else if (reservationState.duration === 'daily') {
            summaryHTML += `<p><strong>تاریخ شروع:</strong> ${reservationState.startDate ? formatJalali(reservationState.startDate) : '-'}</p>`;
            const days = reservationState.dayCount || 1;
            summaryHTML += `<p><strong>مدت:</strong> ${toFarsiNumber(days)} روز</p>`;
        }
        ...
        if (totalPrice > 0) {
            summaryHTML += `<p><strong>مبلغ کل:</strong> ${toFarsiNumber(Math.round(totalPrice).toLocaleString())} تومان</p>`;
        }
        ...
        if (mobileBar && mobileText) {
            ...
            if (totalPrice > 0) {
                text += ` • ${toFarsiNumber(Math.round(totalPrice).toLocaleString())} تومان`;
            }
            mobileText.textContent = toFarsiNumber(text);
    ...
    function renderSpecials() {
        ...
        showSkeleton(specialsGrid, 2);
        setTimeout(() => {
            specialsGrid.innerHTML = '';
            if (specialsData.length === 0) {
                specialsGrid.innerHTML = emptyStateHTML('پیشنهاد ویژه‌ای موجود نیست', 'ready', 'دیدن موتورهای موجود');
                return;
            }
            specialsData.forEach(item => {
                ...
                card.innerHTML = `
                    ...
                    <span class="price">ساعتی ${toFarsiNumber(item.price.toLocaleString())} تومان</span>
    ...
    function renderReadyList() {
        ...
        setTimeout(() => {
            ...
            if (filtered.length === 0) {
                readyGrid.innerHTML = emptyStateHTML('موتوری موجود نیست', 'specials', 'دیدن مدل‌های مشابه');
                return;
            }
    ...
    async function renderReviews() {
        ...
        showSkeleton(reviewsGrid, 3);
        try {
            const res = await fetch('api/reviews.php');
            reviewsData = await res.json();
        } catch (err) {
            reviewsGrid.innerHTML = emptyStateHTML('خطا در بارگذاری نظرات');
            return;
        }
        reviewsGrid.innerHTML = '';
        if (reviewsData.length === 0) {
            reviewsGrid.innerHTML = emptyStateHTML('نظری یافت نشد', 'contact', 'ارسال نظر');
            return;
        }
    ...
    async function renderMemories() {
        ...
        try {
            ...
            container.innerHTML = '';
            if (!list.length) {
                container.innerHTML = emptyStateHTML('عکسی موجود نیست');
                return;
            }
            list.forEach(src => {
            ...
            container.innerHTML = emptyStateHTML('خطا در بارگذاری تصاویر');
            ...
            navigator.geolocation.getCurrentPosition((position) => {
                ...
                (async () => {
                    ...
                    if (NESHAN_SERVICE_API_KEY) {
                        ...
                        displayDist = `${toFarsiNumber(km)} کیلومتر`;
                        const mins = Math.round(minDurationSec / 60);
                        displayTime = `${toFarsiNumber(mins)} دقیقه`;
                    }
                    ...
                    <p>قیمت ساعتی: ${toFarsiNumber(nearestVehicle.price.toLocaleString())} تومان</p>
    ...
    function initIslandMap(motors) {
        ...
        if (!motors || motors.length === 0) {
            mapContainer.innerHTML = emptyStateHTML('موتوری برای نمایش وجود ندارد', 'ready', 'دیدن موتورهای موجود');
            return;
        }
    ...
    function initHeroOverlay(motors) {
        ...
        if (availableCountElem && reservedCountElem) {
            ...
            availableCountElem.textContent = toFarsiNumber(available);
            reservedCountElem.textContent = toFarsiNumber(reserved);
        }
        if (ctaBtn) {
    ...
    function loadLiveMotors() {
        const mapContainer = document.getElementById('island-map-container');
        if (mapContainer) showSkeleton(mapContainer, 1, 'map');
        fetch('api/motors.php')
            ...
            .catch(err => {
                if (mapContainer) mapContainer.innerHTML = emptyStateHTML('خطا در بارگذاری نقشه');
                console.error('خطا در دریافت لیست موتورها', err);
            });
    }
