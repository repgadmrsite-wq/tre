...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['add_motor'])) {
    ...
    $battery = filter_input(INPUT_POST, 'battery', FILTER_VALIDATE_INT, ['options' => ['min_range' => 0]]);
    if ($battery === false) { $errors[] = 'ظرفیت باتری نامعتبر است'; }
    $range = filter_input(INPUT_POST, 'range_km', FILTER_VALIDATE_INT, ['options' => ['min_range' => 0]]);
    if ($range === false) { $errors[] = 'برد نامعتبر است'; }
    $top_speed = filter_input(INPUT_POST, 'top_speed', FILTER_VALIDATE_INT, ['options' => ['min_range' => 0]]);
    if ($top_speed === false) { $errors[] = 'سرعت نهایی نامعتبر است'; }
    $weight = filter_input(INPUT_POST, 'weight', FILTER_VALIDATE_INT, ['options' => ['min_range' => 0]]);
    if ($weight === false) { $errors[] = 'وزن نامعتبر است'; }
    $battery = filter_input(INPUT_POST, 'battery', FILTER_VALIDATE_INT, ['options' => ['min_range' => 0]]);
    if ($battery === false) { $errors[] = 'ظرفیت باتری نامعتبر است'; }
    $range = filter_input(INPUT_POST, 'range_km', FILTER_VALIDATE_INT, ['options' => ['min_range' => 0]]);
    if ($range === false) { $errors[] = 'برد نامعتبر است'; }
    ...
    $stmt = $pdo->prepare('INSERT INTO motorcycles (model, plate, color, capacity, battery_kwh, range_km, top_speed, weight, description, status, price_per_hour, price_half_day, price_per_day, price_per_week, price_per_month, insurance, year, mileage, available, is_special, lat, lng) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)');
    $stmt->execute([$model, $plate, $color, $capacity, $battery, $range, $top_speed, $weight, $description, $status, $price_hour, $price_half, $price_day, $price_week, $price_month, $insurance, $year, $mileage, $available, $is_special, $lat, $lng]);
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['update_motor'])) {
    ...
    $stmt = $pdo->prepare('UPDATE motorcycles SET model=?, plate=?, color=?, capacity=?, battery_kwh=?, range_km=?, top_speed=?, weight=?, description=?, status=?, price_per_hour=?, price_half_day=?, price_per_day=?, price_per_week=?, price_per_month=?, insurance=?, year=?, mileage=?, available=?, is_special=?, lat=?, lng=? WHERE id=?');
    $stmt->execute([$model, $plate, $color, $capacity, $battery, $range, $top_speed, $weight, $description, $status, $price_hour, $price_half, $price_day, $price_week, $price_month, $insurance, $year, $mileage, $available, $is_special, $lat, $lng, $id]);
  ...
  <main class="main-content">
    <div class="container-fluid">
      ...
      <form method="post" enctype="multipart/form-data" class="row g-2 mb-4 card p-3">
        ...
        <div class="col-md-2"><input type="number" name="battery" class="form-control" placeholder="باتری kWh" value="<?= htmlspecialchars($editMotor['battery_kwh'] ?? ''); ?>"></div>
        <div class="col-md-2"><input type="number" name="range_km" class="form-control" placeholder="برد km" value="<?= htmlspecialchars($editMotor['range_km'] ?? ''); ?>"></div>
        <div class="col-md-2"><input type="number" name="top_speed" class="form-control" placeholder="سرعت km/h" value="<?= htmlspecialchars($editMotor['top_speed'] ?? ''); ?>"></div>
        <div class="col-md-2"><input type="number" name="weight" class="form-control" placeholder="وزن kg" value="<?= htmlspecialchars($editMotor['weight'] ?? ''); ?>"></div>
